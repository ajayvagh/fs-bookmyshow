...
router.get('/get-all-shows-by-theatre', async (req, res) => {
    try {
        const shows = await Show.find({ theatre: req.body.theatreId }).populate('movie')
        res.send({
            success: true,
            message: "All shows Found",
            data: shows
        });
...
router.get('/get-all-theatres-by-movie', async (req, res) => {          //was post
    try {
...
router.get('/get-show-by-id', async (req, res) => {         //was post
    try {
